...
def compute_cross_covariance(P, Q, correspondences, kernel=lambda diff: 1.0):
    ...
    for i, j in correspondences:
        ...
        a.append(float(p_point[0]))
        b.append(float(p_point[1]))
        c.append(float(q_point[0]))
        d.append(float(q_point[1]))
    np.array(a)
    np.array(c)
    f = np.cov(a, c)
...
# Move the data
moved_data = (R_true.dot(true_data) + t_true)[:, 21:24]
...
plt.show()
print(norm_values)
test = animate_results(P_values, Q, corresp_values, xlim=(-5, 35), ylim=(-5, 35))
saveHtml("test", bytes(test.data,  encoding="utf8"))